...
#
# نکته‌ی مهم درباره‌ی الگوی دوم («N-»): این الگو برای تشخیص نشانه‌هایی طراحی شده که
# ترتیبشان برعکس شود (کمیاب، ولی دیده شده)، اما دقیقاً همین شکل («عدد» + خط‌تیره)
# با سلول‌های یک جدول «محدوده‌ی صفحه/سؤال» (مثلاً در جدول بودجه‌بندی دفترچه‌های
# قلم‌چی: «۳-۴» یعنی صفحه‌ی ۳ تا ۴، یا «۱-۲۰» یعنی سؤال ۱ تا ۲۰) قابل‌اشتباه‌شدن است.
# وقتی pdfplumber بین خط‌تیره و عدد دومِ محدوده یک فاصله وارد می‌کند (که برای
# ستون‌های جدول رایج است، مثلاً «3- 4»)، عدد اول محدوده به‌غلط به‌عنوان نشانه‌ی
# سؤال شناسایی می‌شود؛ چون این اعداد کاذب معمولاً در همان صفحه‌ی اول (پیش از
# صفحات واقعی سؤال‌ها) ظاهر می‌شوند، اسکن حریصانه‌ی پایین را «جلو می‌اندازند» و
# باعث می‌شوند شماره‌سؤال‌های واقعیِ کوچک (که واقعاً بعداً می‌آیند) به‌اشتباه «قبلاً
# رد شده» تلقی و نادیده گرفته شوند. راه‌حل: برخلاف الگوی اول (که به‌عمد فقط اجازه‌ی
# نداشتن رقم بلافاصله بعد از عدد را می‌دهد، چون متن سؤال واقعی گاهی خودش با یک
# فاصله و یک عدد شروع می‌شود—مثلاً «-6 اگر عدد ۲۵ را...»—و نباید رد شود)، در الگوی
# دوم فاصله‌ی اختیاریِ بعد از خط‌تیره را هم در نظر می‌گیریم: اگر خط‌تیره (با یا بدون
# یک فاصله) بلافاصله با رقمِ دیگری دنبال شود، این را «انتهای یک محدوده» در نظر
# می‌گیریم نه نشانه‌ی سؤال، و رد می‌کنیم.
_MARKER_RE = re.compile(
    r"(?<!\d)-\s?(\d{1,3})(?!\d)"   # -12  یا  - 12
    r"|"
    r"(?<!\d)(\d{1,3})\s?-(?!\s?\d)"   # 12-  یا  12 -   (ولی نه «12- 34» که یعنی محدوده‌ی 12 تا 34)
)
